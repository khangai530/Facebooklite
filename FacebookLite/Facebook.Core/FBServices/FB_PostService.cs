...
{
    private readonly FB_MemoryRepo<FB_Post> _postRepo; //FB_Post turliin entity hadgalna

    public FB_PostService(FB_MemoryRepo<FB_Post> postRepo) //repositoryg constructor-r damjuulj avch baina
    {
        _postRepo = postRepo;
    }

    public FB_Post CreatePost(FB_User author, string content) //textpost uusgeh method
    {
        var post = new FB_TextPost(author, content); //textpost onbject uusgelee
        _postRepo.Add(post); //repositoryd hadgalna
        Console.WriteLine($"✅ Шинэ пост нэмэгдлээ: {author.UserName}");
        return post; //uusgesen post butsaana
    }

    public FB_Post CreatePhotoPost(FB_User author, string content, string photoUrl) //photopost uusgej baina
    {
